function firebaseMockF({ TIMESTAMP_PATH = '/timestamp' } = {}) {
  const firebaseRefOffSpy = jest.fn().mockImplementation((path) => {
    _callbacks[path] = {};
  });
  const firebaseRefOnSpy = jest.fn();
  const serverTimeMock = jest.fn().mockImplementation(() => new Date());
  ...
  const createMockFirebaseRef = (path = '') => ({
    get on() {
      return firebaseRefOnSpy.mockImplementation((event, cb) => {
        _callbacks[path] = {
          ..._callbacks[path],
          [event]: {
            cb,
  ...
  const firebaseMock = {
    initializeApp: jest.fn().mockImplementation(function () {
      return this;
    }),
    delete: jest.fn(),
    ...
    mockServerTime: (time) => {
      ...
      serverTimeMock.mockImplementation(() => time);
    },
    mockDisconnect: () => {
